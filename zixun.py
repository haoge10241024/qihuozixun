...
menu = ["上海金属网-快讯", "百度股市通-期货-新闻", "美联储利率决议报告", "美国 CPI 月率报告", "美国核心 CPI 月率报告", "美国非农就业人数报告", "美国 ADP 就业人数报告", "伦敦金属交易所(LME)-库存报告", "全球宏观指标重大事件", "智能问答"]
choice = st.sidebar.selectbox("选择功能", menu)

if choice == "上海金属网-快讯":
    symbol = st.text_input("请输入想要获取资讯的品种名 (如 '全部', '要闻', 'VIP', '财经', '铜', '铝', '铅', '锌', '镍', '锡', '贵金属', '小金属'):")
...
elif choice == "美国 CPI 月率报告":
    start_year = st.number_input("请输入想要获取数据的起始年代 (如 1990):", min_value=1900, max_value=2100, step=1)
    if st.button("获取数据"):
        data_df = get_cpi_data(start_year)
        st.write(data_df)

elif choice == "美国核心 CPI 月率报告":
    start_year = st.number_input("请输入想要获取数据的起始年代 (如 1990):", min_value=1900, max_value=2100, step=1)
    if st.button("获取数据"):
        data_df = get_core_cpi_data(start_year)
        st.write(data_df)

elif choice == "美国非农就业人数报告":
    start_year = st.number_input("请输入想要获取数据的起始年代 (如 1990):", min_value=1900, max_value=2100, step=1)
    if st.button("获取数据"):
        data_df = get_non_farm_data(start_year)
        st.write(data_df)

elif choice == "美国 ADP 就业人数报告":
    start_year = st.number_input("请输入想要获取数据的起始年代 (如 1990):", min_value=1900, max_value=2100, step=1)
    if st.button("获取数据"):
        data_df = get_adp_employment_data(start_year)
        st.write(data_df)

elif choice == "伦敦金属交易所(LME)-库存报告":
    start_date = st.date_input("请输入开始日期:")
    end_date = st.date_input("请输入结束日期:")
    commodity = st.text_input("请输入品种名 (如 '铜', '锡', '铅', '锌', '铝', '镍'):")
    if st.button("获取数据"):
    ...
    date = st.text_input("请输入日期 (格式: YYYYMMDD):")
    if st.button("获取数据"):
